...
<div class="mx-auto max-w-md">
  <Card class="shadow-xl bg-gradient-to-br from-indigo-500/10 via-purple-500/10 to-pink-500/10 border border-purple-300/40">
    ...
    <CardContent>
      <form method="POST" class="space-y-5" onsubmit={() => isSubmitting = true}>
        <div class="space-y-2">
          <Label for="email" class="text-indigo-700">Email</Label>
          <Input id="email" name="email" type="email" required value={form?.values?.email ?? ""} placeholder="you@company.com" autocomplete="email" class="border-purple-400/60 focus:border-indigo-600 focus:ring-indigo-600 bg-white/80" />
        ...
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            ...
            <a href="/forgot-password" class="text-sm text-purple-700 underline-offset-4 hover:underline">Forgot?</a>
          </div>
          <div class="relative">
            <Input 
              id="password" 
        ...
        <div class="flex items-center gap-2 text-sm">
          <input type="checkbox" id="remember" name="remember" class="h-4 w-4 rounded border-purple-400 text-indigo-700 focus:ring-indigo-600" />
          <Label for="remember" class="text-indigo-700 text-sm">Remember me</Label>
        </div>
      ...
      <p class="mt-4 text-center text-sm text-purple-700/80">
        Need an account?
        <a href="/signup" class="text-indigo-700 underline-offset-4 hover:underline">Create one</a>
      </p>
    </CardContent>
  </Card>
</div>